...
class GuestControllerTest {
	...
	void testGuest_list() throws Exception {
		...
		.andExpect(forwardedUrl("/WEB-INF/views/guest_list.jsp")).andDo(print());
	...
	void testGuest_viewIntModel() throws Exception {
		...
		mockMvc.perform(get("/guest_view").param("guest_no","1")).andExpect(status().isOk())//검증시작
		.andExpect(model().attributeExists("guest")).andExpect(forwardedUrl("/WEB-INF/views/guest_view.jsp")).andDo(print());
